...
@section('content')
  <div class="ui one column doubling stackable grid container">
	  ...
	  @if(Laralum::loggedInUser()->hasPermission('laralum.users.create'))				
	      <a href="{{ route('Laralum::users_create') }}" class="res_ex ui {{ Laralum::settings()->button_color }} button">
  		...
  		<div class="ui very padded segment">
  			<table class="ui table " id="example">
					...
					<tr>
						<td>
                            <div class="text">
                                ...
                                <a href="{{ route('Laralum::user_details',['id' => $user->id, 'name' => $user->name]) }}">{{ $user->name }}</a>
                                @if($user->su)
                ...
                <!--td >
                      ...
                      @endif
					  @if($user->id != Laralum::loggedInUser()->id)
					  <a href="{{ route('Laralum::switch_start',['id' => $user->id]) }}" class="ui {{ Laralum::settings()->button_color }} button">Login</a>
				     @endif
					 <a href="{{ route('Laralum::user_details',['id' => $user->id, 'name' => $user->name]) }}" class="ui {{ Laralum::settings()->button_color }} button">View</a>
					 </td-->
