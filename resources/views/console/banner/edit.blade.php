...
@section('breadcrumb')
    <div class="ui breadcrumb">
        ...
        <div class="active section">{{ trans('laralum.banners_edit_title') }}</div>
    </div>
@endsection
@section('title', trans('laralum.banners_edit_title'))
@section('icon', "edit")
@section('subtitle', trans('laralum.banners_edit_subtitle', ['name' => $banner->title]))
@section('content')
<div class="ui doubling stackable grid container">
    <div class="three wide column"></div>
    <div class="ten wide column">
        <div class="ui very padded segment">
            <form class="ui form" action="{{ route('console::banner_update', [$banner->id]) }}" method="POST" enctype="multipart/form-data">
                {{ csrf_field() }}
                <div class="field">
					<label>{{ trans('laralum.banners_name') }}</label>
					<input type="text" id="title" name="title" value="{{ old('title', isset($banner) ? $banner->title : '') }}" placeholder="{{ trans('laralum.banners_name') }}" />
                 </div>
				 <div class="field">
				    <label>{{ trans('laralum.banners_img') }}</label>
					  <div class="customFile" data-label="Choose File">
						...
						<input type="hidden" id="banner_img_hidden" name="banner_img_hidden" value="{{ $banner->image }}" />
					  </div>
				 </div>
				 
				 <div class="field">
					<label>{{ trans('laralum.banners_desc') }}</label>
					<textarea id="banner_desc" name="banner_desc" rows="4" cols="50">
					{{ old('banner_desc', isset($banner) ? $banner->description : '') }}
					</textarea>
                 </div>
                <br>
