<div class="modal-header" id="attendDetail">
    <div class=""  style="display: inline-block;padding:5px;">
        @if($clock_in > 0)
    ...
    <h4 class="modal-title" style="display: inline-block;padding:5px;"><i class="icon-clock"></i> Attendance
        @if($type == 'edit')
            Details
        @else
            Mark
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group bootstrap-timepicker timepicker">
                                                ...
                                                <input type="text" name="clock_in_time"
                                                       ...
                                                       @if(!is_null($row->clock_in_time)) value="{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $row->clock_in_time)->timezone("Asia/Kolkata")->format("h:i a") }}" @endif>
                                            </div>
                                        </div>

                                        <div class="col-md-3">
                                            <div class="form-group">
                                                <label class="control-label">Clock In IP</label>
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group bootstrap-timepicker timepicker">
                                                ...
                                                <input type="text" name="clock_out_time" id="clock-out"
                                                       ...
                                                       @if(!is_null($row->clock_out_time)) value="{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $row->clock_out_time)->timezone("Asia/Kolkata")->format("h:i a") }}" @endif>
                                            </div>
                                        </div>
...
<script>
    ...
    $('#attendance-container').on('click', '.save-attendance', function () {
        @if($type == 'edit')
            var url = '{{route('Crm::attendance.update', $row->id)}}';
        @else
            var url = '{{route('Crm::attendance.storeMark')}}';
        @endif
        $.easyAjax({
            url: url,
            type: "POST",
