                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="attendance_date" id="attendance_date" value="{{ Carbon\Carbon::now()->timezone('Asia/Kolkata')->format('d-m-Y') }}">
                                        </div>
                                    </div>
                                </div>
...
<script>
    ...
    function checkHoliday() {
        ...
        $.ajax({
            ...
            url:  "{{route('Crm::attendance.check-holiday')}}",
            data: {
                    _token : "{{ csrf_token() }}",
                    date: selectedDate
                },
        ...
        .done(function(data) {
            if(data.status==true){
                if(data.holiday != null){
                    $('#holidayBox').show();
                    $('#tableBox').hide();
                    $('#holidayReason').html(data.holiday.occassion);
                }else{
                    $('#holidayBox').hide();
                    $('#tableBox').show();
                    showTable();
    ...
    $('#attendance-table').on('click', '.save-attendance', function () {
        ...
        $.ajax({
            ...
            url:  "{{route('Crm::attendance.store')}}",
    ...
    function attendanceDetail(id,attendanceDate) {
        var url = "{{ route('Crm::attendance.detail') }}?userID="+id+"&date="+attendanceDate;
