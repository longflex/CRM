                ...
                <div class="card-body">
                    <div class="row bg-title">
                        ...
                        <div class="col-lg-12 col-sm-12 col-md-12 col-xs-12 text-right">
                            ...
                            @if(Laralum::hasPermission('laralum.holiday.create'))
                                <a onclick="showAdd()" class="btn btn-outline btn-success btn-sm ">Add Holiday <i class="fa fa-plus" aria-hidden="true"></i></a>
                            @endif
                                ...
                                <div class="row">
                                    <div class="col-md-12 ">
                                        <div class="form-group col-md-2 pull-right">
                                            ...
                                            <select onchange="showData()" class="select2 form-control" data-placeholder="Projects Status" id="year">
                                                @forelse($years as $yr)
    ...
    function del(id, date) {
        const swalWithBootstrapButtons = Swal.mixin({
            ...
            if (result.isConfirmed) {
                var url = "{{ route('Crm::staff.holidays.destroy',':id') }}";
        ...
        function showAdd() {
            var url = "{{ route('Crm::staff.holidays.create') }}";
            $.ajaxModal('#edit-column-form', url);
        }
        // Show Create Holiday Modal
        function showMarkHoliday() {
            var url = "{{ route('Crm::staff.holidays.mark-holiday') }}";
            $.ajaxModal('#edit-column-form', url);
        }
        ...
        function showData() {
            var year = $('#year').val();
            var url = "{{ route('Crm::staff.holidays.view-holiday',':year') }}"
            url = url.replace(':year', year);
            $.easyAjax({
                type: 'GET',
                url: url,
