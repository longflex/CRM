...
<link href="{{ asset('home_assets/sweetalert/sweetalert.css') }}" rel="stylesheet"> 
...
<div class="container-fluid inner-header termspageheader">
    <div class="container">   
     <div class="row">
      <div class="col-lg-12 col-md-12 col-sm-12">    
        <h1 class="text-uppercase text-left yellowheadingtxt">@yield('title', '')</h1>
      </div>
    </div>
  </div>
</div>
...
<script src="{{ asset('home_assets/js/bootstrap.bundle4.4.min.js') }}"></script> 
<script src="{{ asset('home_assets/sweetalert/sweetalert.min.js') }}"></script> 
<script>
$(document).ready(function() {	
    $('#calculate_form').submit(function(e) {	
	    e.preventDefault();		
		var sms_type = $('#sms_type').val();
		var msg_count = $('#msg_qty').val();
		if(sms_type==''){
		   swal('warning!','Please select sms type!','error')
		   return false;
		}
		if(msg_count==''){
		   swal('warning!','Please enter no. of sms!','error')
		   return false;
		}
		var formData = new FormData(this); 
		var my_url = "{{ url('/calculatePrice') }}";
		var type = "POST"; 
		$.ajaxSetup({
		   headers: {
		...
		$("#hidectext").css('display','none');
	    $(".calculationForm").css('display','inline-block');			    
		$.ajax({
			type: type,
			url: my_url,
			data: formData,
			...
			success: function (data) {
			  if(data.status=='success'){
				$(".calculationForm").css('display','none');
				$("#hidectext").css('display','inline-block');
				$(".show_calculation").css('display','block');
                $("#total_qty").html(data.msg_qty);
                $("#total_price").html(data.price);
				$("#total_amount").html(data.total_amount);	
			  }else{
				swal({ title: "Warning!", text: "Something went wrong, please try agan!", type: "warning" }, function(){ location.reload(); });	  
			  }				
			},
			error: function (data) {
