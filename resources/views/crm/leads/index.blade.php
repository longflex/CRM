...
<div class="ui breadcrumb">
	<div class="active section">{{ trans('laralum.leads_title') }}</div>
</div>
@endsection
@section('title', trans('laralum.leads_title'))
@section('icon', "mobile")
@section('subtitle', trans('laralum.leads_title'))
@section('content')
		...
		<div class="ui very padded segment">
			<div class="row">
				<div class="col-md-12 text-right">
					...
					<span>{{count($leads)}} Member(s) &nbsp &nbsp</span>
					@if(Laralum::hasPermission('laralum.member.create'))
					<a href="{{ route('Crm::leads_create') }}"
					...
					{{-- 
						<a href="{{ route('Crm::leads_export', ['client_id' => $client_id]) }}"
					...
					<div class="filter-area" style="display: none;">
						<div class="row">
							<form class="ui form" method="GET" action="{{ route('Crm::leads') }}">
								...
								<div class="col-md-3">
									<div class="form-group">
										<select class="form-control custom-select" name="member_type"
											id="member_type_id">
											<option value="" selected>All Member Type</option>
							...
							<tr>
								<td>
									<div class="text">
										...
										<a
											href="{{ route('Crm::lead_details', ['id' => $lead->id]) }}">{{ $lead->name }}</a>
										@else{{ $lead->name }}
										@endif
									</div>
								</td>
								<td>
									<div class="text">
										<span class="badge badge-info">{{ $lead->member_id }}</span>
									</div>
								</td>
								<td>
									<div class="text">
...
$("#action_btn").click(function(){
  ...
  $('input[type=checkbox]:checked').each(function(i, val){
	...
	if(ids.length==0){
		...
		swal('Error!', 'Please select member first!', 'error')
		return;
	}
	...
	$('.dimmer').removeClass('dimmer');
		...
		<script src="{{ asset('crm_public/js/leads-script.js') }}" type="text/javascript"></script>
		<script>
 ...
 $(document).on('keyup', '#search_record', function (event) {
	 if (event.keyCode === 13) {
		var query = {
			...
			var url = "{{route('Crm::leads')}}?" + $.param(query)
